...
namespace libfintx
{
    public static class RdhxCrypt
    {
        ...
        public static void Encrypt(string Message, out byte[] encSessionKey, out byte[] encMsg)
        {
            ...
            if (DEBUG.Enabled)
                DEBUG.Write("Public bank encryption key: " + Converter.ByteArrayToString(Encoding.GetEncoding("iso8859-1").GetBytes(RdhKeyStore.KEY_ENCRYPTION_PUBLIC_BANK)));

            encSessionKey = encryptKey(Encoding.GetEncoding("iso8859-1").GetBytes(RdhKeyStore.KEY_ENCRYPTION_PUBLIC_BANK));
        ...
        private static byte[] encryptKey(byte[] Key)
        {
            ...
            var key = Encoding.GetEncoding("iso8859-1").GetBytes(RdhKeyStore.KEY_ENCRYPTION_PUBLIC_BANK);
            ...
            var cryptLength = HbciUtil.CheckForCryptSize(RdhKeyStore.KEY_ENCRYPTION_PUBLIC_BANK.Length, result.Length);
        ...
        {
            // Decrypt session key
            var k = Encoding.GetEncoding("iso8859-1").GetBytes(RdhKeyStore.KEY_ENCRYPTION_PRIVATE);
