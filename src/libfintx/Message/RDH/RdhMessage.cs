...
namespace libfintx
{
    public static class RdhMessage
    {
            ...
            string SystemID, string Segments, int SegmentNum)
        {
            ...
            sigHead = "HNSHK:" + SEGNUM.SETVal(2) + ":4+" + RdhProfile.RDHPROFILE + "+2+" + secRef + "+1+1+1::" + SystemID + "+1+1:" + date + ":" + time +
                "+1:" + Sig.HASHALG_SHA256_SHA256 + ":1+6:" + Sig.SIGALG_RSA + ":" + Sig.SIGMODE_PSS + "+" + SEG_Country.Germany + ":" + RdhKeyStore.Blz + ":" + RdhKeyStore.UserId + ":" + KeyType.Sig + ":" + RdhProfile.Version + ":1'";

            // Sig
            //var sig = Sig.hash(Segments);
            //var sig = Sig.SignDataSHA256(Segments);
            //var signature = Sig.SignMessage(sig);

            //Sig.Verify(sig, signature);

            var signature = Sig.Sign(Segments);

            var v = Sig.Verify(Segments, signature);

            Console.WriteLine("------------------->" + v);

            //var signedsig = Encoding.GetEncoding("iso8859-1").GetString(signature);

            var signedsig = signature;
            ...
            encHead = "HNVSK:" + Enc.SECFUNC_ENC_PLAIN + ":3+" + RdhProfile.RDHPROFILE + "+4+1+1::" + SystemID + "+1:" + date + ":" + time + "+2:2:" + Enc.ENCALG_2K3DES + ":@" +
               ...
               KeyType.Enc + ":" + RdhProfile.Version + ":1+0'";
