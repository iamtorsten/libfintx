...
namespace libfintx.FinTS
{
    public class HKCDN
    {
        ...
        public static async Task<String> Init_HKCDN(FinTsClient client, string OrderId, string Receiver, string ReceiverIBAN, string ReceiverBIC, decimal Amount, string Usage, DateTime FirstTimeExecutionDay, TimeUnit timeUnit, string Rota, int ExecutionDay, DateTime? LastExecutionDay)
        {
            ...
            SEG sEG = new SEG();
            string segments = sEG.toSEG("HKCDN", client.SEGNUM, 1, 0, connectionDetails.Iban + sEG.Finisher +
            ...
            segments += sEG.Delimiter + sEG.Delimiter + OrderId + sEG.Delimiter + FirstTimeExecutionDay.ToString("yyyyMMdd") + sEG.Finisher + (char) timeUnit + sEG.Finisher + Rota + sEG.Finisher + ExecutionDay;
